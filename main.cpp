...
namespace utils {
    ...
    void saveToFile(const Patient Db[], int numpatients, const string& filename = "patient_db.txt") {
        ofstream outFile(filename);
        if (outFile.is_open()) {
            // First write the number of patients
            outFile << numpatients << "\n";

            // Then write each patient's data
            for (int i = 0; i < numpatients; ++i) {
                outFile << Db[i].name << "\n";
                outFile << Db[i].age << "\n";
                outFile << Db[i].weight << "\n";
                outFile << Db[i].gender << "\n";
                outFile << Db[i].phoneNumber << "\n";
                outFile << Db[i].bloodType << "\n";
                outFile << Db[i].previousConditions << "\n";
            }
            outFile.close();
        } else {
            cerr << "Error: Unable to save database to file.\n";
        }
    }
    // Load database from file
    void loadFromFile(Patient Db[], int& numpatients, const string& filename = "patient_db.txt") {
        ifstream inFile(filename);
        if (inFile.is_open()) {
            // First read the number of patients
            inFile >> numpatients;
            inFile.ignore(); // Ignore the newline

            // Then read each patient's data
            for (int i = 0; i < numpatients; ++i) {
...
void addPatient(Patient Db[], int& numpatients) {
    ...
    cout << "--------------------------------------------------\n";
    utils::saveToFile(Db, numpatients);
...
void editpatient(Patient Db[],int numpatients){
    ...
    if (patientId >= 1 && patientId <= numpatients) {
        ...
        utils::saveToFile(Db, numpatients); // Save the updated database to file
        cout << "Database updated.\n";
    } else {
...
int main() {
    ...
    utils::loadFromFile(Db, numpatients);
